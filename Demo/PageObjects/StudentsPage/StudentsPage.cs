...
namespace Demo.PageObjects
{
    class StudentsPage : BasePage
    {
        ...
        [FindsBy(How = How.CssSelector, Using = "img[title='Students']")]
        private IWebElement iconStudents;
        [FindsBy(How = How.Id, Using = "ctl00_ContentPlaceHolder1_lblTitle")]
        private IWebElement AssertLogo;

        //CHECKBOX CLIENT GROUP SELECTION
        [FindsBy(How = How.Id, Using = "ctl00_ContentPlaceHolder1_ddlClientGroup_Arrow")]
        private IWebElement ClientGroupSelection;
        [FindsBy(How = How.XPath, Using = "//div[@id='ctl00_ContentPlaceHolder1_ddlClientGroup_DropDown']/div/ul/li")]
        private IList<IWebElement> ClientGroup;
        ...
        public void navigateToStudentspage(string reportTab, string LoginpageUrl)
        {
            ...
            try
            {
                ClickElement(iconStudents);
                ReportLog("Passed", "Clicked on Students icon");
            }
            catch (Exception ex)
            {
                ReportLog("Failed", "Didn't click on students icon");
                throw ex;
            }
            try
            {
        ...
        public void selectClientGroupDropdown(String expClientGroup)
        {
            ClickElement(ClientGroupSelection);
            Sleep(3000);
            foreach (IWebElement clientGroup in ClientGroup)
            {
                if (clientGroup.Text.Equals(expClientGroup))
                {
                    ClickElement(clientGroup);
                    break;
                }
            }
            Sleep(1000);
